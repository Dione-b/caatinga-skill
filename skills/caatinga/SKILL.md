...
| Assume browser invoke supports multisig | Treat browser invoke as single-invoker only | Caatinga does not implement multisig; multi-auth is application-owned, and contracts requiring delegated AddressV2 / non-invoker `signAuthEntry` credentials fail with `CAATINGA_MULTI_AUTH_REQUIRED`. Recommending a multisig flow here will produce a broken or misleading UX. |
